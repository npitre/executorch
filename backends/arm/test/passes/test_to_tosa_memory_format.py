...
class NHWCSafeSpatialMerge(torch.nn.Module):
    ...
    preserved.  ``is_channel_reshape`` returns False (NHWC-safe: monotonic
    shape_indices with batch and channel alone), so no transposes are inserted
...
def test_channel_reshape_nhwc_safe():
    """Shapes already NHWC by the time the pass runs.

    [1,2,14,72] -> [1,28,1,72], sr=2 -> NHWC-safe (spatial merge, C=72
    preserved).  is_channel_reshape should return False (no transposes needed).
